...
プロジェクトルート（`AI_Memorizer/`）に `.env.local` というファイルを**新規作成**し、以下の内容を記入します。
...
**「Deploy」** ボタンをクリックします。
1〜2 分後にビルドが完了し、`https://ai-memorizer-xxxx.vercel.app` のような URL が発行されます。
...
3. 以下を設定して保存
   ...
   | URL | Vercel の公開 URL（例: `https://ai-memorizer-xxxx.vercel.app`） |
...
`.env.local` ファイルがプロジェクトルート（`AI_Memorizer/`）に存在するか確認してください。
